...
//			}
		...
		void PositingUpdating(edge[][] X,edge[][] V,int N,edge[][] E,int[][] len)//����Xi
		{
			...
			int Tstart=TTstart;
			tmp[TTstart]=1;
			int Tend=FindNextNode(Tstart,cutV[Tstart],X[Tstart],E[Tstart],tmp,N,random);
			tmp[Tend]=1;
			...
			for(;;)
			{
				...
				Tstart=Tend;
				Tend=FindNextNode(Tstart,cutV[Tstart],X[Tstart],E[Tstart],tmp,N,random);
				tmp[Tend]=1;
		...
		static int FindNextNode(int Start,edge[] cutV,edge[] X,edge[] E,int[] tmp,int N,Random random)
		{
			int k;
			...
			if( AvailableEdgeNum>0)//������Դ�v�л�ȡ�Ļ�
			{
				int select=random.nextInt( AvailableEdgeNum);//��ʾѡ��ڼ������õı�
				int ii=0;
				for(k=0;k<N-1;k++)
				{
					if(cutV[k].pe>0 && tmp[cutV[k].end]==0)
			...
			else
			{
				...
				if(tmp[X[select].end]==0)
				{
					FindEdge.copy(X[select]);
					//tmp[FindEdge.end]=1;
				}
				else if(tmp[X[(select+1)%2].end]==0)
				{
					FindEdge.copy(X[ (select+1)%2 ]);
				//	tmp[FindEdge.end]=1;
				}
				//���涼�������e��ѡ���
				else
				{
					select=random.nextInt(N-1);
					while(tmp[E[select].end]==1)
					{select=random.nextInt(N-1);}
					FindEdge.copy(E[select]);
					//tmp[FindEdge.end]=1;
				}
			}
			return FindEdge.end;
